...
|2|1|`ENABLE_DEST_INDEX`|If `true`, causes [`SFPSWAP`](SFPSWAP.md) to perform `argmin` and `argmax` rather than `min` and `max`; while this bit is set, instructions other than `SFPLOAD`, `SFPLOADI`, `SFPSWAP`, or `SFPTRANSP` that write `LReg[4..7]` are `UnsupportedFunctionality` due to Wormhole/Blackhole HW erratum `TEN-2932` (fixed in Quasar)|
